...
1. **`SameSite=Lax` eliminates the traditional CSRF vector.** Modern browsers
   ...
   form submissions.
2. **No GET endpoints mutate state.** All mutations use POST/PUT/PATCH/DELETE, so
   the GET exception in `SameSite=Lax` is irrelevant.
...
This aligns with the Epic Stack's decision to remove CSRF (epicweb-dev/epic-stack
decision 035).

## Consequences

- If a future endpoint uses `SameSite=None` cookies or mutates state via GET, CSRF
  protection must be re-evaluated for that endpoint.
